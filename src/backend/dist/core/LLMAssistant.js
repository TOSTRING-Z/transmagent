...
class LLMAssistant {
    llm_service;
    plugins;
    constructor(llm_service, plugins = null) {
    ...
    async setChatName(_data = {}) {
        ...
        const history_content = this.llm_service.chatManager.messages
            ...
            .map(m => globals_1.utils.parseJsonContent(m.content)?.content || "")
            .join("===");
    ...
    async auditToolCall(toolInfo, data) {
        ...
        if (!globals_1.utils.getConfig("tool_call")?.llm_judge) {
            return null;
        }
  ...
  "reason": "Required if pass is false"
    ...
    async checkConsoleOutput(consoleOutput, executionTimeMs = 0) {
        try {
            ...
            const lastAssistantIdx = allMessages.map(m => m.role).lastIndexOf('assistant');
            if (lastAssistantIdx === -1) {
                logger_1.logger.warn("[Critic] 未找到对应的助手消息，跳过检查");
...
`.trim();
            const callData = react_agent.getDataDefault({
                query,
                params: { ...globals_1.utils.getConfig("llm_params"), temperature: 0.1 },
                push_message: true,
                output_format: null
            });
            if (!callData.params)
            ...
            if (messageOutput && messageOutput.content) {
                ...
                if (jsonMatch) {
                    const verdict = JSON.parse(jsonMatch[0]);
                    logger_1.logger.log(`[ConsoleMonitor] Interrupt check: shouldInterrupt=${verdict.shouldInterrupt}, reason=${verdict.reason}`);
                    return {
