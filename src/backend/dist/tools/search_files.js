...
function main() {
    return async ({ path: targetPath, regex = "", file_pattern = "**/*", timeout_ms = 30000 }) => {
        ...
        try {
            ...
            for (const file of validFiles) {
                if (fileProcessingTimedOut)
                    break;
                // 检查文件遍历是否超时
                if (Date.now() - startTime > timeout_ms) {
                    logger_1.logger.warn(`Search interrupted: Timed out after ${timeout_ms}ms. Partial results returned.`);
                ...
                try {
                    ...
                    logger_1.logger.warn(`Skipping file due to read error: ${file}, Error: ${readError.message}`);
                    continue;
                }
                ...
                while ((match = regexObj.exec(content)) !== null) {
                    // 检查正则匹配循环是否超时
                    if (Date.now() - startTime > timeout_ms) {
                        logger_1.logger.warn(`Search interrupted during regex execution: Timed out after ${timeout_ms}ms.`);
                        regexTimedOut = true;
                        break;
                    }
            ...
            logger_1.logger.error(`Search files error: ${error.message}`);
