...
class BackgroundTaskRegistry {
     ...
     * 但保持存活，可以继续接收消息。
     */
    static addAgentCompletionNotice(sessionId, taskId, content) {
        const task = this.tasks.get(taskId);
        if (task) {
            task.status = 'idle';
            task.endTime = Date.now();
            task.resultSummary = content.replace(/\n/g, ' ').substring(0, 200);
            logger_1.logger.log(`[BackgroundTaskRegistry] Task "${taskId}" marked as idle (sub-agent alive)`);
        }
        const msg = {
            type: 'task_result',
            taskId,
     ...
     */
    static queueAgentMessage(sessionId, agentName, msg) {
        const key = `${sessionId}::${agentName}`;
        if (!this.agentMsgQueues.has(key)) {
            this.agentMsgQueues.set(key, []);
        }
        this.agentMsgQueues.get(key).push(msg);
        logger_1.logger.log(`[BackgroundTaskRegistry] Queued agent message for "${agentName}" (active), ` +
