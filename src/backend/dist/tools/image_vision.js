    ...
    return async (args) => {
        try {
            ...
            if (['.png', '.jpg', '.jpeg', '.webp'].includes(ext)) {
                ...
                logger_1.logger.info(`Rendering PDF via Offline Puppeteer + local pdf.js...`);
                let browser;
                let tempHtmlPath = "";
                try {
                    ...
                    if (errorMsg)
                        throw new Error("Local PDF.js Render Error: " + errorMsg);
                    // 5. 截图拔线
                    finalPngBase64 = await page.screenshot({ encoding: 'base64', type: 'png', fullPage: true });
                }
                catch (err) {
