import { ChatState, Message } from '../types';
export declare const hashCode: (str: string) => string;
export declare const extractJson: (text: string) => string | null;
export declare const parseJsonContent: (content: string) => any;
export declare const delay: (seconds: number) => Promise<void>;
export declare const getDefault: (name?: string) => string;
export declare const getSystem: (name?: string) => string;
export declare const getFile: (file_path: string) => string | null;
export declare const mergeConfigEnhanced: (defaultConfig: any, userConfig: any) => {
    mergedConfig: any;
    mismatches: any[];
    addedKeys: any[];
};
export declare const getLanguage: () => string;
export declare const formatDate: () => string;
export declare const copy: <T>(data: T) => T;
export declare const getSessionId: () => string;
export declare const getDefaultConfig: (key?: string | null) => any;
export declare const getDefaultHistoryConfigPath: () => string;
export declare const getDefaultHistoryPath: (id: string) => string;
export declare const readJsonFile: (filePath: string) => any;
export declare const writeFile: (filePath: string, data: string | object) => Promise<void>;
export declare const deleteFile: (filePath: string) => boolean;
export declare const delHistoryChat: (id: string) => void;
export declare const getHistoryChat: (id: any) => ChatState | undefined;
export declare const getHistoryMessages: (id: any) => Message[] | undefined;
export declare const setHistory: (chat: any, messages?: Message[] | undefined) => boolean;
export declare const setHistoryMessages: (chatId: any, messages: any) => boolean;
export declare const setHistoryChat: (chat: any) => boolean;
export declare const setHistoryConfigChat: (chat: ChatState) => boolean;
export declare const setDefaultConfig: (config: Record<string, any>, configName?: string) => boolean;
/**
 * 检查静默模式是否启用
 * @returns boolean - 静默模式状态，默认返回 false
 */
export declare const isSilentMode: () => boolean;
