...
console.log('\n📋 测试1: AgentListener 注册与注销');
{
    ...
    const listener = (msg: any) => { receivedMessages.push(msg); };

    // 注册
    BackgroundTaskRegistry.registerAgentListener(sessionId, agentName, listener);

    // 发送消息
    BackgroundTaskRegistry.addAgentMessage(sessionId, 'worker_b', 'worker_a', 'Hello from B!');
    ...
    // 注销
    BackgroundTaskRegistry.unregisterAgentListener(sessionId, agentName);
    BackgroundTaskRegistry.addAgentMessage(sessionId, 'worker_b', 'worker_a', 'Second message');
...
console.log('\n📋 测试2: addAgentMessage 路由 — target="main"');
{
    ...
    // 清除之前可能残留的 pending
    BackgroundTaskRegistry.addAgentMessage(sessionId, 'worker_a', 'main', '任务完成！');
...
console.log('\n📋 测试2b: addAgentMessage 路由 — target="all" 广播');
{
    ...
    BackgroundTaskRegistry.registerAgentListener(sessionId, 'agent_a', (m) => msgsA.push(m));
    BackgroundTaskRegistry.registerAgentListener(sessionId, 'agent_b', (m) => msgsB.push(m));

    BackgroundTaskRegistry.addAgentMessage(sessionId, 'controller', 'all', '广播消息');
    ...
    BackgroundTaskRegistry.unregisterAgentListener(sessionId, 'agent_a');
    BackgroundTaskRegistry.unregisterAgentListener(sessionId, 'agent_b');
}
...
console.log('\n📋 测试2c: addAgentMessage 路由 — target=特定代理（离线排队）');
{
    ...
    // 目标代理未注册 → 应排队
    BackgroundTaskRegistry.addAgentMessage(sessionId, 'worker_a', agentName, '离线消息');

    // 后来注册 → 应立即收到排队消息
    const msgs: any[] = [];
    BackgroundTaskRegistry.registerAgentListener(sessionId, agentName, (m) => msgs.push(m));
...
console.log('\n📋 测试3: drainAgentMessages 排空机制（无 listener 时排队）');
{
    ...
    BackgroundTaskRegistry.addAgentMessage(sessionId, 'sender1', agentName, 'msg1');
    BackgroundTaskRegistry.addAgentMessage(sessionId, 'sender2', agentName, 'msg2');
    BackgroundTaskRegistry.addAgentMessage(sessionId, 'sender3', agentName, 'msg3');

    // drainAgentMessages 返回排队消息并清除
    const drained = BackgroundTaskRegistry.drainAgentMessages(sessionId, agentName);
    assert(drained.length === 3, 'drain 应返回 3 条排队消息');
    assert(drained[0].content === 'msg1', '第一条排队消息正确');
    assert(drained[2].content === 'msg3', '最后一条排队消息正确');

    // 再次 drain 应为空
    const drained2 = BackgroundTaskRegistry.drainAgentMessages(sessionId, agentName);
...
console.log('\n📋 测试4: send_message 工具逻辑（通过 BackgroundTaskRegistry 验证）');
{
    ...
    const msgs: any[] = [];
    BackgroundTaskRegistry.registerAgentListener(sessionId, toAgent, (m) => msgs.push(m));

    // 模拟 send_message 内部调用
    const msgContent = '请基于此数据生成报告';
    BackgroundTaskRegistry.addAgentMessage(sessionId, fromAgent, toAgent, msgContent);

    assert(msgs.length === 1, '定向消息应被投递');
    assert(msgs[0].from === fromAgent, '来源应为 data_processor');
    assert(msgs[0].content === msgContent, '内容应匹配');

    BackgroundTaskRegistry.unregisterAgentListener(sessionId, toAgent);
}
...
console.log('\n📋 测试5: 多代理网状通信拓扑');
{
    ...
    for (const [name, box] of Object.entries(inbox)) {
        BackgroundTaskRegistry.registerAgentListener(sessionId, name, (m) => box.push(m));
    }

    // researcher → analyst
    BackgroundTaskRegistry.addAgentMessage(sessionId, 'researcher', 'analyst', '原始数据已就绪');
    // researcher → writer
    BackgroundTaskRegistry.addAgentMessage(sessionId, 'researcher', 'writer', '请准备报告模板');
    // analyst → writer
    BackgroundTaskRegistry.addAgentMessage(sessionId, 'analyst', 'writer', '分析结果：p<0.05');
    // writer → all (报告完成广播)
    BackgroundTaskRegistry.addAgentMessage(sessionId, 'writer', 'all', '报告已完成！');
    ...
    for (const name of Object.keys(inbox)) {
        BackgroundTaskRegistry.unregisterAgentListener(sessionId, name);
    }
}
...
console.log('\n📋 测试6: 并发消息投递');
{
    ...
    for (let i = 0; i < count; i++) {
        BackgroundTaskRegistry.addAgentMessage(sessionId, `sender_${i}`, agentName, `message_${i}`);
    }
...
console.log('\n📋 测试7: 跨 Session 隔离');
{
    ...
    const msgsA: any[] = [];
    const msgsB: any[] = [];
    BackgroundTaskRegistry.registerAgentListener(sessionA, agentName, (m) => msgsA.push(m));
    BackgroundTaskRegistry.registerAgentListener(sessionB, agentName, (m) => msgsB.push(m));

    BackgroundTaskRegistry.addAgentMessage(sessionA, 'sender', agentName, 'message for A');

    assert(msgsA.length === 1, 'Session A 应收到消息');
    assert(msgsB.length === 0, 'Session B 不应收到消息');

    BackgroundTaskRegistry.unregisterAgentListener(sessionA, agentName);
    BackgroundTaskRegistry.unregisterAgentListener(sessionB, agentName);
}
