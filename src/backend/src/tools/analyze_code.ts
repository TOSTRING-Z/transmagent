...
function analyzeFile(filePath, language = 'js') {
  ...
  try {
      ...
      return {
        ...
        details: {
          ...
          classes: phpClasses.map(cls => ({
            ...
            methods: [] as any // PHP类方法需要额外解析，这里留空
      ...
      return {
        ...
        details: {
          ...
          classes: pythonClasses.map(cls => ({
            ...
            methods: [] as any // Python类方法需要额外解析，这里留空
    ...
    const parserConfig: { sourceType: string; plugins: string[] } = {
    ...
    const result: {
      file: string;
      language: string;
      summary: { classes: number; functions: number; variables: number; imports: number };
      details: {
        classes: Array<{ name: any; line: number; methods: any }>;
        functions: Array<{ name: any; line: number; type: string }>;
        variables: Array<{ name: any; line: number; type: any }>;
        imports: Array<{ source: any; line: number; specifiers: any }>;
      };
    } = {
