...
const fetch_search_1 = require("./fetch_search");
...
describe('Fetch & Search Tools', () => {
    describe('TTLCache', () => {
        it('应该能正常存储并获取未过期的值', () => {
            const cache = new fetch_search_1.TTLCache(10, 60);
            cache.set('key1', 'value1');
            expect(cache.get('key1')).toBe('value1');
        });
        it('当超过 maxSize 时应该驱逐最早的键', () => {
            const cache = new fetch_search_1.TTLCache(2, 60);
            cache.set('k1', 'v1');
            cache.set('k2', 'v2');
            cache.set('k3', 'v3');
            // k1 应该被移除
        ...
        it('当 ttl 过期时应该返回 undefined', () => {
            // 使用 Jest 假时间
            jest.useFakeTimers();
            const cache = new fetch_search_1.TTLCache(10, 1); // 1秒过期
            cache.set('k', 'v');
    ...
    describe('WebBrowser Orchestration', () => {
        let browser;
        beforeEach(() => {
            browser = new fetch_search_1.WebBrowser();
            // Mock Searcher
            ...
            // Mock Fetcher
            jest.spyOn(browser.fetcher, 'fetch').mockImplementation(async (url) => {
    ...
    describe('Main Action Router', () => {
        let actionRunner;
        beforeEach(() => {
            actionRunner = (0, fetch_search_1.main)({ searcher_type: 'DuckDuckGoSearch' });
...
//# sourceMappingURL=fetch_search.test.js.map