...
class ErrorSolutionFinder {
    ...
    async waitForVerificationComplete(page) {
        logger_1.logger.log('🔍 检测到验证页面，请手动完成验证...');
        logger_1.logger.log('💡 提示: 完成验证后，页面会自动跳转到搜索结果');
        // 可选：通知前端
        if (WindowManager_1.WindowManager?.instance?.alertWindow) {
            WindowManager_1.WindowManager.instance.alertWindow.show("log", "Please manually complete the verification");
        }
        ...
        while (Date.now() - startTime < timeout) {
            try {
                ...
                logger_1.logger.log(`当前URL: ${currentUrl}`);
                if (currentUrl !== lastUrl) {
                    logger_1.logger.log('🔄 检测到页面跳转...');
                    lastUrl = currentUrl;
                    consecutiveStableChecks = 0;
                    if (currentUrl.includes('/search') &&
                        !currentUrl.includes('nocaptcha') &&
                        !currentUrl.includes('challenge')) {
                        logger_1.logger.log('✅ 已跳转到搜索结果页面');
                ...
                if (pageState.hasResults && !pageState.stillVerifying && pageState.isSearchPage) {
                    logger_1.logger.log('✅ 验证完成，检测到搜索结果');
                    await new Promise(resolve => setTimeout(resolve, 3000));
                    return true;
                }
                if (consecutiveStableChecks > 3 && !pageState.stillVerifying) {
                    logger_1.logger.log('🔄 URL稳定，尝试检查是否验证完成...');
                    if (consecutiveStableChecks > 6) {
                        logger_1.logger.log('🔄 尝试重新加载页面...');
                ...
                logger_1.logger.log('⏳ 等待验证完成...');
            }
            catch (error) {
                logger_1.logger.log(`⚠️ 检查过程中出现错误: ${error.message}`);
                await new Promise(resolve => setTimeout(resolve, 2000));
            }
        }
        logger_1.logger.log('⏰ 验证等待超时，尝试继续...');
        return false;
    }
    async crawlStackOverflow(searchQuery, maxResults = 5) {
        ...
        try {
            ...
            logger_1.logger.log(`🔍 正在搜索 Stack Overflow: ${searchQuery}`);
            logger_1.logger.log(`🌐 搜索URL: ${searchUrl}`);
            page.on('console', msg => {
                if (msg.type() === 'error')
                    logger_1.logger.log('❌ 页面错误:', msg.text());
            });
            page.on('response', response => {
                if (response.status() >= 400) {
                    logger_1.logger.log('⚠️ 响应错误:', String(response.status()), response.url());
                }
            });
            await page.goto(searchUrl, { waitUntil: 'networkidle2', timeout: 30000 });
            const initialUrl = await page.url();
            logger_1.logger.log(`📍 初始页面URL: ${initialUrl}`);
            ...
            if (needsVerification) {
                logger_1.logger.log('🛡️ 需要验证，等待手动完成...');
                const verificationSuccess = await this.waitForVerificationComplete(page);
                if (!verificationSuccess) {
                    logger_1.logger.log('⚠️ 验证可能未完成，尝试继续...');
                }
            }
            logger_1.logger.log('📊 开始提取搜索结果...');
            const results = await this.extractSearchResults(page, maxResults);
            logger_1.logger.log(`✅ 从 Stack Overflow 找到 ${results.length} 个结果`);
            return results;
        }
        catch (error) {
            logger_1.logger.error(`❌ Stack Overflow 爬取错误: ${error.message}`);
    ...
    async extractSearchResults(page, maxResults) {
        try {
            await new Promise(resolve => setTimeout(resolve, 5000));
            const results = await page.evaluate((maxAllowed) => {
                const solutions = [];
                const selectors = [
                    '.s-post-summary',
                    '.question-summary',
                ...
                questionElements.forEach((element) => {
                    if (solutions.length >= maxAllowed)
                        return;
            ...
            logger_1.logger.error(`❌ 提取搜索结果错误: ${error.message}`);
            return [];
        }
    }
    ...
    async getSolutionUrls(errorMessage, maxResults = 5) {
        try {
            ...
            logger_1.logger.log(`🚀 开始搜索错误解决方案: "${errorMessage}"`);
...
//# sourceMappingURL=error_solution_search.js.map