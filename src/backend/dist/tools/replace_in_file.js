...
// 写入远程文件内容
async function writeRemoteFile(filePath, content, sshConfig) {
    return new Promise((resolve, reject) => {
        const conn = new ssh2_1.Client();
        conn.on('ready', () => {
            conn.sftp((err, sftp) => {
...
// 瘦身后的本地文件替换逻辑
function replaceInLocalFile(file_path, diff) {
...
// 瘦身后的远程文件替换逻辑
async function replaceInRemoteFile(file_path, diff, sshConfig) {
