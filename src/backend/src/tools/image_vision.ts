...
export function main(params: VisionParams) {
    return async (args: ToolArgs): Promise<string> => {
        try {
            ...
            if (!prompt) return "Error: 'prompt' argument is required.";
            if (!file_path) return "Error: 'file_path' argument is required.";

            const apiUrl = params.api_url || "https://api.openai.com/v1/chat/completions";
            const apiKey = params.api_key;
            const model = params.model || "gpt-4o";

            if (!apiKey) return "Error: 'api_key' is missing in tool configuration.";
            ...
            try {
                if (isRemote) {
                    logger.info(`Reading remote file via SFTP: ${file_path}`);
            ...
            if (!fileBuffer || fileBuffer.length === 0) return "Error: File is empty.";
            ...
            if (['.png', '.jpg', '.jpeg', '.webp', '.gif'].includes(ext)) {
                logger.info(`Native image format detected. Skipping browser rendering.`);
                finalPngBase64 = fileBuffer.toString('base64');
            } else {
                // 如果是 PDF, SVG, HTML 等需要渲染的文件，启动浏览器
                logger.info(`Complex format (${ext}) detected. Launching Headless Browser...`);
                let browser;
                let tempFilePath = "";
                ...
                try {
                    ...
                    logger.info(`Browser navigating to file...`);
                    ...
                    finalPngBase64 = await page.screenshot({ encoding: 'base64', type: 'png' }) as string;
                    logger.info(`Browser screenshot successful.`);

                } catch (browserErr: any) {
                    logger.error(`Browser rendering failed: ${browserErr.message}`);
