 ...
 */
function createBackgroundMessageMiddleware(getSessionId, injectMessage) {
    return async (ctx, next) => {
        ...
        for (const pending of pendingMessages) {
            if (pending.type === 'task_result') {
                logger_1.logger.log(`[BackgroundMsgMiddleware] Injecting background task result "${pending.taskId}" ` +
                    `into session "${sessionId}"`);
            }
            else if (pending.type === 'agent_message') {
                logger_1.logger.log(`[BackgroundMsgMiddleware] Injecting agent message into session "${sessionId}"`);
            }
