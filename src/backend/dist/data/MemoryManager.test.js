...
jest.mock('../utils/public', () => ({
    getDefault: jest.fn((name) => `/tmp/.transmagent/${name ?? ''}`),
    getDefaultConfig: jest.fn(),
}));
// ── 正式 import ──────────────────────────────────────────────────────────────
const axios_1 = __importDefault(require("axios"));
const MemoryManager_1 = __importDefault(require("./MemoryManager"));
const MemoryDB_1 = require("./MemoryDB");
const public_1 = require("../utils/public");
// ── 类型辅助 ─────────────────────────────────────────────────────────────────
const mockedAxios = axios_1.default;
const MockedMemoryDB = MemoryDB_1.MemoryDB;
const mockedGetDefaultConfig = public_1.getDefaultConfig;
// ── 测试数据 ──────────────────────────────────────────────────────────────────
...
describe('MemoryManager.queryLongTermMemory', () => {
    ...
    beforeEach(() => {
        ...
        manager = new MemoryManager_1.default();
    });
    // ─────────────────────────────────────────────────────────────────────────
    ...
    describe('Embedding 服务可用', () => {
        ...
        it('应调用 axios 获取 Embedding 向量', async () => {
            ...
            expect(mockedAxios.post).toHaveBeenCalledWith(expect.stringContaining('/embeddings'), expect.objectContaining({ input: '今天天气如何' }), expect.any(Object));
        });
        it('应将 embedding 和 query 传入 memoryDB.query', async () => {
            await manager.queryLongTermMemory('今天天气如何');
            expect(mockQueryFn).toHaveBeenCalledTimes(1);
            expect(mockQueryFn).toHaveBeenCalledWith(FAKE_EMBEDDING, '今天天气如何', 5);
        });
        ...
        it('自定义 topK 应正确透传', async () => {
            ...
            expect(mockQueryFn).toHaveBeenCalledWith(FAKE_EMBEDDING, '会议安排', 3);
        });
    });
    // ─────────────────────────────────────────────────────────────────────────
    ...
    describe('Embedding API 请求失败', () => {
        ...
        it('axios 失败后应以 null embedding 调用 memoryDB.query', async () => {
            ...
            expect(mockQueryFn).toHaveBeenCalledWith(null, '网络异常测试', 5);
        });
    });
    // ─────────────────────────────────────────────────────────────────────────
    // 场景 4：默认 topK 参数验证
