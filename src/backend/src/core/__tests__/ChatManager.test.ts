...
describe('ChatManager.fixMessages', () => {
    const createToolCall = (id: string, name: string) => ({
        id,
    ...
    describe('场景2: 最后一条是 assistant，有 tool_calls 但无 tool 结果', () => {
        it('应该补充缺失的 tool 结果', () => {
            const messages = [
                createMessage({ 
                    role: 'assistant', 
            ...
            const toolMsgs = chatManager.messages.filter(m => m.role === 'tool');
    ...
    describe('场景3: 最后一条是 assistant，有 tool_calls 且有部分 tool 结果', () => {
        it('应该只补充缺失的 tool 结果', () => {
            const messages = [
                createMessage({ 
                    role: 'assistant', 
    ...
    describe('场景4: 最后一条是 tool，tool 结果数量少于 tool_calls 数量', () => {
        it('应该补充缺失的 tool 结果', () => {
            const messages = [
                createMessage({ 
                    role: 'assistant', 
            ...
            expect(toolMsgs[2].content).toBe('The user interrupted the task.');
        });
    });

    describe('场景5: 完整的工具调用链（无需修复）', () => {
        it('应该保持消息不变', () => {
            const messages = [
                createMessage({ 
                    role: 'assistant', 
    ...
    describe('场景6: assistant 没有 tool_calls', () => {
        it('应该保持消息不变', () => {
            const messages = [
                createMessage({ 
                    role: 'user', 
    ...
    describe('场景7: 空消息列表', () => {
        it('应该不报错', () => {
            ...
            expect(() => chatManager.fixMessages()).not.toThrow();
            expect(chatManager.messages.length).toBe(0);
        });
    });

    describe('场景8: 只有 user 消息被删除后的场景', () => {
        it('最后一条是 assistant 且 tool_calls 已完整时应保持不变', () => {
            const messages = [
                createMessage({ 
                    role: 'user', 
