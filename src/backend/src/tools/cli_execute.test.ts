import { main, getPrompt } from './cli_execute';
...
describe('cli_execute tool', () => {
    ...
    describe('Local Execution', () => {
        it('1. 应该能够成功执行本地 bash 代码并返回结果', async () => {
            ...
            const execMock = (child_process.exec as unknown as jest.Mock).mock.results[0].value;
    ...
    describe('SSH Execution', () => {
        it('3. 当开启 SSH 配置时，应通过 ssh2 模块上传并执行代码', async () => {
            (utils.getSshConfig as jest.Mock).mockReturnValue({ enabled: true, host: '192.168.1.1' });
            ...
            const connMock = (Client as unknown as jest.Mock).mock.results[0].value;
    ...
    describe('IPC Resource Cleanup', () => {
        it('4. 无论成功失败，结束时都必须清理 IPC 监听器防止内存泄漏', async () => {
            ...
            expect(ipcMain.removeListener).toHaveBeenCalledWith('close-window', expect.any(Function));
            expect(ipcMain.off).toHaveBeenCalledWith('terminal-input', expect.any(Function));
        });
    });
    ...
    describe('Threshold Truncation', () => {
        it('5. 输出过长时应该被截断', async () => {
            ...
            const execute = main({ max_lines: 10, max_chars_per_line: 100 });
