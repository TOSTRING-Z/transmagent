...
class BrowserController {
    ...
    async navigateToUrl(url, options = {}) {
        ...
        try {
            ...
            logger_1.logger.log(`导航完成: ${pageInfo.title}`);
    ...
    async executeJavaScript(jsCode, options = {}) {
        ...
        try {
            logger_1.logger.log('执行JavaScript代码...');
            ...
            logger_1.logger.log('JavaScript执行完成');
    ...
    async executePuppeteerAction(action, params) {
        ...
        try {
            logger_1.logger.log(`执行Puppeteer操作: ${action}`);
            let result = null;
            ...
            switch (action) {
                ...
                case 'evaluate':
                    const evalFunc = typeof params.function === 'string'
                        ? new Function(`return (${params.function})()`)
                        : params.function;
            ...
            logger_1.logger.log(`Puppeteer操作 ${action} 执行完成`);
            return {
                success: true,
    ...
    setupEventListeners() {
        ...
        this.page.on('console', msg => {
            logger_1.logger.log('浏览器控制台:', msg.type(), msg.text());
        });
        this.page.on('pageerror', error => {
            logger_1.logger.log('页面错误:', error);
        });
        this.page.on('requestfailed', request => {
            logger_1.logger.log('请求失败:', request.url(), request.failure()?.errorText);
        });
    }
    getBrowserStatus() {
        return {
...
class ContentExtractor {
    ...
    async executePuppeteerAction(params) {
        const { action, waitAfterAction = 1000, wait_after_action, ...actionParams } = params;
...
if (require.main === module) {
    (async () => {
        try {
            logger_1.logger.log('=== 测试内容提取器（支持Puppeteer原生操作）===\n');
            const testExtractor = new ContentExtractor();
            logger_1.logger.log('1. 打开浏览器...');
            let result = await testExtractor.main({ operation: 'open' });
            logger_1.logger.log('打开结果:', result.success ? '成功' : '失败');
            if (!result.success)
                return;
            ...
            logger_1.logger.log('Navigate 结果:', result.success ? '成功' : '失败');
            logger_1.logger.log('\n2. 获取页面内容...');
            result = await testExtractor.main({
            ...
            logger_1.logger.log('内容提取结果:', result.success ? '成功' : '失败');
            logger_1.logger.log('\n3. 测试Puppeteer滚动操作...');
            result = await testExtractor.main({
            ...
            logger_1.logger.log('滚动操作:', result.success ? '成功' : '失败');
            logger_1.logger.log('\n4. 测试获取元素信息...');
            result = await testExtractor.main({
                operation: 'get_element_info',
                selector: 'h1'
            });
            logger_1.logger.log('元素信息:', result.success ? '成功' : '失败');
            if (result.success) {
                logger_1.logger.log('元素存在:', result.data.exists);
            }
            logger_1.logger.log('\n5. 测试截图操作...');
            result = await testExtractor.main({
                operation: 'puppeteer_action',
            ...
            logger_1.logger.log('截图操作:', result.success ? '成功' : '失败');
            if (result.success) {
                logger_1.logger.log('截图数据长度:', result.data.result.data.length);
            }
            logger_1.logger.log('\n6. 关闭浏览器...');
            result = await testExtractor.main({ operation: 'close' });
            logger_1.logger.log('关闭结果:', result.success ? '成功' : '失败');
        }
        catch (error) {
