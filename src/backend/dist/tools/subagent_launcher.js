...
async function runSubAgentInBackground(taskId, parentSessionId, agentName, agentPrompt, query, toolNames, timeout, parentUtils, mainToolCall, mainLLMService, model, version, agentMode, toolFormat, parentMode) {
    ...
    try {
        ...
        BackgroundTaskRegistry_1.BackgroundTaskRegistry.registerAgentListener(parentSessionId, agentName, (msg) => {
            ...
            if (subAgentToolCall.state !== LLMBase_1.State.IDLE &&
                ...
                BackgroundTaskRegistry_1.BackgroundTaskRegistry.requeueForMiddleware(subAgentToolCall.llmService.chatManager.chat.id, pendingMsg);
                return;
            }
            // ── 空闲状态：直接注入 + 唤醒 ──
    ...
    const idleCheckInterval = setInterval(async () => {
        const pendingMsgs = BackgroundTaskRegistry_1.BackgroundTaskRegistry.drainMessages(subAgentToolCall.llmService.chatManager.chat.id);
        if (pendingMsgs && pendingMsgs.length > 0) {
            logger_1.logger.log(`[SubAgentLauncher] Agent "${agentName}" idle watcher found ${pendingMsgs.length} pending message(s)`);
            for (const pm of pendingMsgs) {
                const msgs = subAgentToolCall.llmService.chatManager.messages;
                const last = msgs[msgs.length - 1];
                if (last) {
            ...
            try {
                ...
                logger_1.logger.error(`[SubAgentLauncher] Agent "${agentName}" idle wake-up error:`, err);
            }
        }
    }, 2000); // 每 2 秒检查一次
    await new Promise(() => {
