...
- **seaborn**: Alternative data visualization tool
  - Use: `python -c 'import seaborn as sns; import matplotlib.pyplot as plt; tips = sns.load_dataset("tips"); sns.boxplot(x="day", y="total_bill", data=tips); plt.savefig("plot.svg", format="svg")'`  

The above only shows key tool information. To check if other tools are available (using which):
Example: which fastq-dump
