...
/** 心跳消息关键字前缀 */
const HEARTBEAT_KEYWORD = '[SYSTEM HEARTBEAT @';
/**
...
- Just the raw name text.`;
    ...
    // ==================== 工具审计功能 ====================
    isToolRequireAudit(toolName, toolCall) {
            ...
            if (messageOutput?.content && !this.llmService.stopFlag) {
                ...
                if (jsonMatch) {
                    ...
                    if (verdict.pass === false) {
                        ...
                        logger_1.logger.log(`[Critic] 拦截! 建议: ${suggestion}`);
                        return suggestion;
                    }
                }
            }
...
`.trim();
            ...
            const messageOutput = await react_agent.llmCall(callData);
            if (messageOutput?.content && !this.llmService.stopFlag) {
                let organizedContent = messageOutput.content.trim();
            ...
            logger_1.logger.warn(`[MemoryOrganizer] Failed: ${error.message}`);
        }
    }
    // ==================== 心跳任务审查 ====================
    /**
     ...
     */
    detectHeartbeat(query) {
        return typeof query === 'string' && query.startsWith(HEARTBEAT_KEYWORD);
    }
    /**
     ...
     */
    resolveHeartbeatReview(toolInfos, messages) {
        const hasTool = toolInfos.some(t => t.tool_call_name);
        const hasError = toolInfos.some(t => t.error);
        // 本轮有工具/错误 → 直接放行
        if (hasTool || hasError) {
            logger_1.logger.log('[HeartbeatGuard] Heartbeat passed (current round has tool/error).');
            return false;
        }
        ...
        logger_1.logger.log('[HeartbeatGuard] Heartbeat passed (previous rounds had tool calls, preserved for context).');
        return false;
    }
    /**
     ...
     */
    hasToolCallsAfterLastHeartbeat(messages) {
