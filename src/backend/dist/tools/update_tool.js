...
function main() {
    return async (params) => {
        try {
            ...
            // 安全获取 prompt 配置文件路径
            const config = globals_1.utils.getConfig("tool_call") || {};
            const prompt_file = config.cli_prompt || globals_1.utils.getDefault("cli_prompt.md");
            ...
            if (toolStartIndex !== -1) {
                logger_1.logger.log('找到现有工具，进行更新...');
                // 构建替换后的内容
                ...
                logger_1.logger.log('未找到现有工具，添加到文件末尾...');
            ...
            logger_1.logger.error(`Update tool failed: ${error.message}`);
            return {
                success: false,
                error: error.message
            };
...
//# sourceMappingURL=update_tool.js.map