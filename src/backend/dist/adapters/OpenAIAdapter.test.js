...
describe('OpenAIAdapter Advanced Methods', () => {
    ...
    describe('buildPayload', () => {
        ...
        it('当包含 tools 时，应该正确注入 tools 和 tool_choice', () => {
            ...
            const result = adapter.buildPayload(data, []);
    ...
    describe('truncatedResponse (Async Continuation)', () => {
        it('应该能成功续写被截断的 ToolCall JSON 参数', async () => {
            // 1. 准备 Mock 数据环境
            const mockBody = { model: "deepseek-chat", messages: [{ role: 'user', content: 'What is the weather?' }] };
            const mockHeaders = { 'Content-Type': 'application/json', 'Authorization': 'Bearer sk-xxxx' };
            ...
            // });
            // 3. 执行测试
            await adapter.truncatedResponse(mockBody, mockHeaders, mockWindow, mockChatManager, mockMessageOutput, mockData);
            // 4. 验证断言
            ...
            // 验证截断的 JSON 是否被完美拼接
            expect(mockMessageOutput.tool_calls[0].function.arguments[mockMessageOutput.tool_calls[0].function.arguments.length - 1]).toBe('}');
            console.log(mockMessageOutput.tool_calls[0].function.arguments);
